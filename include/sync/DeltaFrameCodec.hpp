...
namespace vectma {
enum class MessageType : uint8_t { NodeAdded = 0, NodeUpdated = 1, NodeRemoved = 2 };
struct MessageHeader { MessageType type; ClientId originClientId; LamportTimestamp tick; };
...
public:
    ...
    struct DecodedMessage { MessageHeader header; NodeId nodeId; uint32_t propertyMask; std::vector<uint8_t> payload; };
    static DecodedMessage decode(const std::vector<uint8_t>& buffer);
};
namespace PropertyFlags { const uint32_t Position = 1 << 0; const uint32_t Color = 1 << 1; }
}
